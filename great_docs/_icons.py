...
# -- Lucide icon path data (v1.7.0) ------------------------------------
#
# Keys are Lucide icon names (lowercase, hyphenated). Values are lists of
# SVG child-element strings (without the outer <svg> wrapper).
#
# Complete set: 1941 icons
...
def get_icon_svg(name: str, size: int = 16, css_class: str = "gd-nav-icon") -> str:
    """
    Return a complete inline ``<svg>`` string for the named Lucide icon.

    Parameters
    ----------
    name
        Lucide icon name (e.g. ``'book-open'``, ``'rocket'``).
    size
    ...
    -------
    str
        Full ``<svg>`` markup ready for inline insertion, or an empty
        string if the icon name is not found.
    """
    elements = LUCIDE_ICONS.get(name)
    if not elements:
        return ""
    ...
    return (
        ...
        f' class="{css_class}" aria-hidden="true">{children}</svg>'
    )


def list_icons() -> list[str]:
    """Return a sorted list of all available icon names."""
    return sorted(LUCIDE_ICONS.keys())
