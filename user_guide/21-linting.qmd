...
| `ambiguous-xref` | Error | An interlink names a short form that two documented objects claim |
...
The same check reports an interlink whose short form two objects claim. If both `StoreCache` and
`NetCache` document a `flush` method, then `` [](`flush`) `` names neither of them, and the
reference renders unlinked:
...
Writing `` [](`StoreCache.flush`) `` resolves it. Docstrings and the pages of your documentation
are both scanned.
