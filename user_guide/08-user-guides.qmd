...
Great Docs treats user guides as first-class content. When you add a `docs/user_guide/` directory to
...
1. create a `docs/user_guide/` directory beside `docs/great-docs.yml`
2. add `.qmd` files for each page
3. run `great-docs build`

Great Docs automatically:

- copies files to `docs/_quarto/default/user-guide/`
...
`docs/user_guide/` directory, the User Guide section appears in your site.
...
your-project/
├── docs/_quarto/default/              # Build directory (ephemeral, gitignored)
│   ├── user-guide/          # Copied from docs/user_guide/
│   └── ...
├── docs/user_guide/              # Your source files (committed to git)
│   ├── 00-introduction.qmd
│   ├── 01-installation.qmd
│   ├── 02-quickstart.qmd
...
The `docs/user_guide/` directory is the default location. You can use a
[custom directory](#custom-user-guide-directory) by setting `user_guide` in `docs/great-docs.yml`.
...
```{.default filename="docs/user_guide/"}
docs/user_guide/
...
docs/user_guide/
├── 01-guide.qmd
├── images/          # Copied to docs/_quarto/default/user-guide/images/
│   ├── logo.png
│   └── diagram.svg
└── data/            # Copied to docs/_quarto/default/user-guide/data/
    └── example.json
```
...
By default, Great Docs looks for `docs/user_guide/`. If you need your
User Guide source files in a different location (e.g., another directory under `docs/` or a monorepo
subdirectory), you can specify a custom path in `docs/great-docs.yml`:

```{.yaml filename="docs/great-docs.yml"}
user_guide: guides
```

The path is relative to the configuration directory. Any directory structure works, including nested paths:
...
If both a `user_guide` config option **and** a conventional `docs/user_guide/` directory exist, the
config option takes precedence and the `docs/user_guide/` directory is ignored. Great Docs will print a
warning to let you know:

```
⚠️  Both 'user_guide' config option ('guides') and 'user_guide/' directory exist; using
configured path
```
...
Whether you use the default `docs/user_guide/` directory or a custom path, Great Docs copies files to
`docs/_quarto/default/user-guide/` during each build, keeping your source separate from generated output.
