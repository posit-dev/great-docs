...
Great Docs provides several ways to create clickable links between related API items. These links help users navigate your documentation and discover connected functionality.
...
You can create inline links to other API items anywhere in your docstring text using the interlinks syntax:

````markdown
[](`~package_name.ClassName`)
````

The `~` prefix tells Great Docs to display only the short name (e.g., `ClassName` instead of the fully qualified `package_name.ClassName`).
...
class DuckDBStore(BaseStore):
    ...
    Inherits from [](`~mypackage.BaseStore`).
    Use [](`~mypackage.query`) to search the store after loading data.
    """
    ...
```

In the rendered documentation, each `[](`~...`)` reference becomes a clickable link pointing to the target's reference page, with the short name displayed as the link text.
...
::: {.callout-tip}
Interlinks require the target to exist in your package's `objects.json` inventory, which is generated automatically during the build. Only items that are documented as part of your API reference can be linked to.
:::

### How Cross-References Are Resolved

All three mechanisms (`%seealso`, NumPy See Also, and interlinks) go through the same resolution pipeline:

1. **During build** — Great Docs generates an `objects.json` inventory mapping every documented symbol to its reference page URI
2. **During post-render** — Interlink references and See Also entries are resolved against this inventory
