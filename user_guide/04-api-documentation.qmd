...
## Great Docs Linking System (GDLS)

Great Docs provides a linking system that automatically creates clickable navigation between your API reference pages. GDLS operates at three levels:

1. **See Also sections** — structured links between related items (`%seealso` directive or NumPy-style sections)
2. **Inline interlinks** — Markdown link syntax to reference other API items anywhere in prose
3. **Code autolinks** — inline code like `` `MyClass` `` is automatically linked when it matches a documented symbol

All three mechanisms resolve against the same `objects.json` inventory that Great Docs generates during the build. To preview which symbols are available before building, run `great-docs scan`.
...
### `%seealso` with Descriptions

You can add a short description after each name, separated by a colon:
...
When descriptions are present, the See Also section renders as a list with each link followed by its description. Without descriptions, the links appear as a compact comma-separated line.
...
If your docstrings use the NumPy docstring format, you can write a standard See Also section instead of (or in addition to) the `%seealso` directive:
...
Great Docs recognizes this format, preserves the descriptions, and merges it with any `%seealso` entries on the same page. Duplicate references are deduplicated automatically.

### Inline Interlinks

You can create inline links to other API items anywhere in your docstring text using the interlinks syntax. There are several forms:
...
The `~` prefix tells Great Docs to display only the short name when no custom text is provided. When you supply custom link text in the brackets, it's always used as-is regardless of the `~` prefix.

This is particularly useful for class hierarchies and overview docstrings:
...
In the rendered documentation, each interlinks reference becomes a clickable link pointing to the target's reference page.

Interlinks work in any part of a docstring: the summary line, extended description, parameter descriptions, notes, or any other section.

### Code Autolinks

Great Docs automatically converts inline code in docstrings into clickable links when the code matches a documented API symbol. No special syntax is needed, you just use standard backtick code formatting:
...
For qualified names, you can control the display text with `~~` prefixes:

| Style | Syntax | Displays as |
| --- | --- | --- |
| **Full path** | `` `mypackage.MyClass` `` | `mypackage.MyClass` |
| **Full path with parens** | `` `mypackage.my_func()` `` | `mypackage.my_func()` |
| **Shortened** | `` `~~mypackage.MyClass` `` | `MyClass` |
| **Shortened with parens** | `` `~~mypackage.my_func()` `` | `my_func()` |
| **Dot-prefixed short** | `` `~~.mypackage.MyClass` `` | `.MyClass` |
| **Unmatched (no link)** | `` `~~unknown_func()` `` | `unknown_func()` |

#### What Gets Autolinked

Any inline code that looks like an identifier or dotted path is a candidate for autolinking. Parentheses at the end are allowed:

**Linked** (if name exists in API):

- `` `MyClass` ``
- `` `my_func()` ``
- `` `mypackage.MyClass` ``

**Not linked** (contains arguments, spaces, or operators):
...
This is the same `{.gd-no-link}` directive used by the [link checker](link-checker.qmd#skipped) to skip URL validation.

::: {.callout-tip}
All GDLS features require the target to exist in your package's `objects.json` inventory, which is generated automatically during the build. Only items that are documented as part of your API reference can be linked to.
:::

### How GDLS Resolves Links

All GDLS mechanisms (`%seealso`, NumPy See Also, interlinks, and code autolinks) go through the same resolution pipeline:

1. **During build** — Great Docs generates an `objects.json` inventory mapping every documented symbol to its reference page URI
2. **During post-render** — References are resolved against this inventory
3. **Name matching** — Both exact matches (`mypackage.MyClass`) and suffix matches (`MyClass`) are supported
4. **Unresolved references** — Names that can't be resolved are shown as plain text (no broken links)
...
Sphinx roles like `:py:class:`, `:func:`, and `:exc:` are converted into plain inline code formatting:

| Source (in docstring) | Rendered output |
| --- | --- |
| `` :py:exc:`ValueError` `` | `ValueError` |
| `` :py:class:`datetime.datetime` `` | `datetime.datetime` |
| `` :func:`my_function` `` | `my_function()` |
| `` :meth:`MyClass.run` `` | `MyClass.run()` |

Function and method roles automatically receive trailing `()` to indicate they are callable.
