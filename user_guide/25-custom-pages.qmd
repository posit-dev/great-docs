...
If you do not set `custom_pages`, Great Docs uses `custom/` beside the configuration:
...
This reads pages from `docs/marketing/` and publishes them under `marketing/` in the built site.
...
This reads files from `docs/marketing/` but publishes them under `py/`, so the source directory and URL can have different names.
...
Custom pages give you an escape hatch for content that doesn't fit the standard page model. Use them sparingly for landing pages, dashboards, or anything that needs full control over HTML and layout.
