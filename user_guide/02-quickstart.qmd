...
You only need to run this **once**. It creates `great-docs.yml` with
sensible defaults. After that, `great-docs build` is the only command
you need.
...
Open `great-docs.yml` and tailor it to your project—organize API
sections, add authors or funding info, set a `display_name`, add a
`user_guide` directory, etc.
See [Configuration](configuration.qmd) for all available options.
...
- **[Configuration](configuration.qmd)** – Customize Great Docs behavior
- **[API Documentation](api-documentation.qmd)** – Understand how API discovery works
