...
1. Create `docs/blog/` with each post in its own subdirectory:
...
Each blog post lives in its own subdirectory under `docs/blog/`, with an `index.qmd` file containing the post content. The configured `dir: blog` is relative to `docs/great-docs.yml`.
