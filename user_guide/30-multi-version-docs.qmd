...
Only Git **tags** are accepted, not branches or arbitrary SHAs. This security restriction ensures that introspection uses immutable references to reviewed, released code. Great Docs caches the results in `docs/.cache/snapshots/`, so later builds do not repeat the introspection. The first build is slower; later builds run without further configuration.
