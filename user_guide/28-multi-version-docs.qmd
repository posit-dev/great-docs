...
Pages with no `versions` key appear in **all** versions. Pages with `versions: [...]` appear only in the listed versions so they are excluded from the sidebar, search index, and build output for other versions. This keeps the navigation clean — users never see links to pages that don't exist in their version.
