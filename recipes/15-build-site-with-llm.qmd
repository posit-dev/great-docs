...
The agent will run `great-docs build` followed by `great-docs preview`. The build generates all reference pages, processes user guides, and renders the site to `docs/_site/`. The preview server opens a local URL (usually `http://localhost:8765`) where you can browse the result.
...
The agent will create a numbered `.qmd` file in `docs/user_guide/` with the right frontmatter and section structure. Great Docs discovers new files automatically on the next build, so no additional configuration is needed.
